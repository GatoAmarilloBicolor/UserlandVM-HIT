...
status_t GuestElfLoader::Load(const char* path,
                              ...
                              SymbolResolver* symbol_resolver)
{
	...
	printf("[ELFLoader] Entry point: 0x%08x, Program headers: %d, Type: %d\n", 
		e_entry, e_phnum, e_type);
	
	fclose(f);

	// DELEGACIÓN: Si es ET_DYN, usar DynamicLinker
	if (e_type == 3) {  // ET_DYN
		printf("[ELFLoader] DYNAMIC BINARY (ET_DYN) - delegating to DynamicLinker\n");
		...
		// Setup stack (increase size to account for local variables and function arguments)
		// Use a larger stack to account for parameters above stackBase
		uint32_t stackBase = 0xC0000000 - 32768;  // Leave 32KB above for parameters/arguments
		uint32_t stackSize = 4 * 1024 * 1024 + 32768;  // 4 MB + 32KB extra
		uint32_t stackOffset = 128 * 1024 * 1024;
		// Register one extra page to handle address calculations slightly above stackBase
		addressSpace.RegisterMapping(stackBase - stackSize, stackOffset, stackSize + 4096);

		// Setup TLS area for guest
		printf("[+] Setting up TLS area for guest\n");
		status_t tls_status = TLSSetup::Initialize(addressSpace, 1);  // thread_id = 1
		if (tls_status != B_OK) {
			fprintf(stderr, "[ELFLoader] Warning: TLS setup failed, continuing anyway: %s\n", 
				strerror(tls_status));
		...
		printf("[ELFLoader] Stack initialized: argc=1, argv[0]=\"%s\", ESP=0x%08x\n", 
			prog_name, stack_ptr);
	...
	// This ensures all binaries (ET_EXEC and libraries) use non-overlapping memory
	...
	for (uint16_t i = 0; i < e_phnum; i++) {
		...
		printf("[ELFLoader] PT_LOAD: vaddr=0x%08x (orig) → 0x%08x (with base), filesz=%u, memsz=%u\n",
		...
		if (p_memsz > p_filesz) {
			...
			while (to_fill > 0) {
				uint32_t fill_amount = (to_fill > sizeof(zeros)) ? sizeof(zeros) : to_fill;
	...
	uint32_t stackBase = 0xC0000000 - 32768;  // Leave 32KB above for parameters/arguments
	uint32_t stackSize = 4 * 1024 * 1024 + 32768;  // 4 MB + 32KB extra
	// FIXED: Stack offset should be dynamically allocated using the global allocator
	// This ensures the stack doesn't overlap with code/data segments
	...
	printf("[ELFLoader] ELF loaded successfully: entry=0x%08x (actual: 0x%08x), stack=0x%08x\n", 
...
status_t GuestElfLoader::ProcessRelocations(FILE* f, AddressSpace& addressSpace,
	uint32_t load_base, uint32_t rel_addr, uint32_t rel_size,
	uint32_t rel_entry_size, SymbolResolver* symbol_resolver,
	const char* strtab, const Elf32_Sym* symtab, uint32_t symcount)
{
	...
	for (uint32_t i = 0; i < num_relocations; i++) {
		...
		if (fread(&r_offset, 4, 1, f) != 1) break;
		if (fread(&r_info, 4, 1, f) != 1) break;
		...
		if (r_type == R_386_RELATIVE) {
			...
			if (i < 5) {  // Print first few for debugging
				printf("[ELFLoader] R_386_RELATIVE: offset=0x%08x, value=0x%08x (base=0x%08x, addend=0x%08x)\n",
			...
			if (symbol_resolver && symtab && strtab && r_sym < symcount) {
				...
				if (sym.st_name > 0 && strtab) {
					...
					if (reloc_value != 0) {
						...
						printf("[ELFLoader] Warning: Could not resolve %s symbol '%s', using NULL\n",
							(r_type == R_386_GLOB_DAT) ? "GLOB_DAT" : "JMP_SLOT",
							sym_name);
					}
				}
