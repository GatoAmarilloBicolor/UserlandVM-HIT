...
def test_dangerous_syscall():
    try:
        ...
        print('SUCCESS: Exception thrown (protection working)')
...
def test_injection(input_str, test_name):
    try:
        ...
        elif 'admin' in input_str or '<script>' in input_str:
...
echo "Testing canary protection..."
...
if tester.test_heap_corruption():
    print(f'ERROR: Use-after-free vulnerability detected!')
else:
...
echo "Testing comprehensive security monitoring..."
...
echo "✅ Security audit test script completed successfully!"