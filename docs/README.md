...
## 2. Mecanismo de Ejecución Integrado (x86-32)

La ejecución de binarios invitados de 32 bits ahora se maneja completamente dentro del proceso principal `UserlandVM`. Esto se logra mediante:

*   **Carga ELF Interna:** La lógica de `GuestElfLoader` (y código similar) se ejecuta dentro del proceso principal de 64 bits.
*   **Contexto Virtualizado:** El CPU virtual (`VirtualCpuX86Native` / `X86_32GuestContext`) inicializa directamente el estado x86 de 32 bits y comienza la ejecución, eliminando la necesidad de un proceso puente separado.
*   **Syscalls Emuladas:** Las llamadas al sistema del binario invitado son interceptadas y manejadas por `Haiku32SyscallDispatcher` dentro del proceso anfitrión.
