...
#include <OS.h> // Incluido para el tipo 'area' y status_t
#include <kernel/OS.h> // Incluido explícitamente para asegurar la definición de 'area'
...
public:
	...
	virtual status_t Read(uint32_t guestAddress, void* buffer, size_t size) override;
	virtual status_t Write(uint32_t guestAddress, const void* buffer, size_t size) override;
	virtual status_t ReadString(uint32_t guestAddress, char* buffer, size_t bufferSize) override;
	...
	virtual status_t RegisterMapping(uint32_t guest_vaddr, uint32_t guest_offset, size_t size) override;
	...
	virtual status_t ReadMemory(uint32_t guest_vaddr, void* data, size_t size) override;
	virtual status_t WriteMemory(uint32_t guest_vaddr, const void* data, size_t size) override;
