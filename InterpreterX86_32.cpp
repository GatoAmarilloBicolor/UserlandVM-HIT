...
static status_t GetModRM_Operand(AddressSpace& space, X86_32Registers& regs,
                                 const uint8* instr, uint32& value, uint32& instr_len)
{
	uint8 modrm = instr[0];
	uint8 mod = (modrm >> 6) & 3;
	uint8 rm = modrm & 7;
...
{
	if (fOptimizedExecutor) delete fOptimizedExecutor;
}
...
status_t InterpreterX86_32::Run(GuestContext& context)
{
	...
	while (instr_count < MAX_INSTRUCTIONS) {
		...
		if (status != B_OK) {
			...
			printf("[INTERPRETER] Instruction execution failed at EIP=0x%08x opcode=0x%02x: status=%d (0x%08x)\n", 
...
status_t InterpreterX86_32::ExecuteInstruction(GuestContext& context, uint32& bytes_consumed)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();

	// Leer instrucción desde dirección virtual (EIP)
	// AddressSpace::Read espera dirección virtual, internamente será traducida si hay mappings
	...
	status_t status = fAddressSpace.Read(regs.eip, instr_buffer, sizeof(instr_buffer));
	if (status != B_OK) {
	...
	// It should include the full instruction length (opcode + operands + prefix if any)
	// The initial value here is just a placeholder
	...
	if (prefix_offset == 0 && !has_fs_override && fOptimizedExecutor) {
		status_t opt_status = fOptimizedExecutor->Execute(x86_context, bytes_consumed);
	...
	switch (opcode) {
		...
		{
			// const char* regs[] = {"EAX", "ECX", "EDX", "EBX", "ESP", "EBP", "ESI", "EDI"}; // Moved to global static array
			DebugPrintf("MOV $imm, %%%s\n", reg_names[opcode - 0xB8]);
			uint32 instr_len = 0;
			status_t status = Execute_MOV(context, &instr_buffer[prefix_offset], instr_len);
		...
		case 0x89:
		{
			...
			if (mod == 3) {
				...
				status_t status = Execute_MOV_Store(context, &instr_buffer[prefix_offset], instr_len);
				bytes_consumed = prefix_offset + instr_len;
				return status;
			}
		}
		...
		case 0x8B:
		{
			...
			if (mod == 3) {
				...
				status_t status = Execute_MOV_Load_FS(context, &instr_buffer[prefix_offset], instr_len);
				bytes_consumed = prefix_offset + instr_len;
				return status;
			} else {
				// Memory load: MOV reg, [mem]
				DebugPrintf("MOV %%r32, [mem]\n");
				status_t status = Execute_MOV_Load(context, &instr_buffer[prefix_offset], instr_len);
				bytes_consumed = prefix_offset + instr_len;
				return status;
			}
		}
		...
		case 0xD3:
		{
			...
			if (mod != 3) {
				...
				uint32 instr_len = 2;
				if (mod == 1) instr_len = 3;
				if (mod == 2) instr_len = 6;
				if (rm_field == 4 && mod != 3) instr_len++;  // SIB
				bytes_consumed = prefix_offset + instr_len;
				return B_OK;
			}
			...
			switch (reg_op) {
				...
				case 7:  // SAR - Arithmetic shift right
					...
					if (shift_count > 0) {
						if ((int32)value < 0) {
							// Sign extend
							result = (value >> shift_count) | ((0xFFFFFFFFUL << (32 - shift_count)) & 0xFFFFFFFFUL);
						} else {
							result = value >> shift_count;
						}
					}
		...
		case 0x84:
		{
			...
			if (mod != 3) {
				...
				uint32 instr_len = 2;
				if (mod == 1) instr_len = 3;  // [base+disp8]
				if (mod == 2) instr_len = 6;  // [base+disp32]
				bytes_consumed = prefix_offset + instr_len;
				return B_OK;
			}
			...
			regs.eflags = 0;
			if (result == 0) regs.eflags |= 0x40;  // ZF
			if ((int32_t)result < 0) regs.eflags |= 0x80;  // SF
		...
		case 0x85:
		{
			...
			if (result == 0) regs.eflags |= 0x40;  // Set ZF
			if (result & 0x80000000) regs.eflags |= 0x80;  // Set SF
		...
		case 0x86:
		{
			...
			if (mod == 3) {
				...
				*reg_ptrs[reg_field] = temp;
				...
				bytes_consumed = prefix_offset + 2;
				if (mod == 1) bytes_consumed += 1;  // [base+disp8]
				if (mod == 2) bytes_consumed += 4;  // [base+disp32]
				return B_OK;
			}
		}
		...
		case 0xAA:
		{
			...
			if (st != B_OK) {
				// Don't treat as fatal error - some programs use STOSB with invalid addresses
				// in initialization code. Log warning but continue.
				DebugPrintf("[INTERPRETER] STOSB: Failed to write to EDI=0x%08x (continuing)\n", regs.edi);
		...
		case 0xA1:
		{
			if (has_fs_override) {
				...
				status_t status = Execute_MOV_Load_FS(context, &instr_buffer[prefix_offset], local_bytes);
				if (status == B_OK) {
					bytes_consumed = prefix_offset + local_bytes;  // prefix + instruction length
				}
				return status;
			} else {
				...
				if (status != B_OK) {
					...
					printf("[INTERPRETER] Warning: Failed to read from 0x%08x in MOV moffs32, treating as zero\n", offset);
					regs.eax = 0;
				}
		...
		case 0xFF:
		{
			...
			if (reg_field == 0) {
				...
				printf("       [0xFF GROUP] Unknown sub-opcode: reg_field=%u - treating as 2-byte NOP\n", reg_field);
				// Unknown 0xFF sub-opcode, skip it gracefully (at least 2 bytes: opcode + modrm)
				// This prevents crashes on undefined opcodes
				bytes_consumed = 2;
		...
		case 0x10:
		{
			...
			if (mod == 3) {
				...
				*reg_ptrs[rm_field] = (*reg_ptrs[rm_field] & 0xFFFFFF00) | result;
				...
				regs.eflags = 0;
				if (result == 0) regs.eflags |= 0x40; // ZF
				if ((int8)result < 0) regs.eflags |= 0x80; // SF
				if ((uint32)dst + (uint32)src + (uint32)cf > 0xFF) regs.eflags |= 0x01; // CF
				bytes_consumed = prefix_offset + 2;
				return B_OK;
			}
		...
		case 0x18:
		{
			...
			if (mod == 3) {
				...
				*reg_ptrs[rm_field] = (*reg_ptrs[rm_field] & 0xFFFFFF00) | result;
				...
				if (result == 0) regs.eflags |= 0x40; // ZF
				if ((int8)result < 0) regs.eflags |= 0x80; // SF
				if ((uint32)dst < (uint32)src + (uint32)cf) regs.eflags |= 0x01; // CF
				bytes_consumed = prefix_offset + 2;
				return B_OK;
			}
			// Memory mode not implemented
		...
		// Note: 0B is actually "OR r32, r/m32" but we also need 0D for "OR eax, imm32"
		// For now, treat 0B as a pass-through
		case 0x0B:
		{
		...
		case 0x3B:
		{
			...
			if (mod == 3) {
				...
				if (result == 0) regs.eflags |= 0x40; // ZF
				if ((int32)result < 0) regs.eflags |= 0x80; // SF
				if (dst_val < src_val) regs.eflags |= 0x01; // CF
				bytes_consumed = prefix_offset + 2;
				return B_OK;
			}
			// Memory mode not yet implemented
		...
		case 0x2C:
		{
			...
			if (result == 0) regs.eflags |= 0x40; // ZF
			if ((int8)result < 0) regs.eflags |= 0x80; // SF
			if ((uint32)al < (uint32)imm8) regs.eflags |= 0x01; // CF
			bytes_consumed = prefix_offset + 2;
			return B_OK;
		}
		...
		case 0x14:
		{
			...
			if (result == 0) regs.eflags |= 0x40; // ZF
			if ((int8)result < 0) regs.eflags |= 0x80; // SF
			if ((uint32)al + (uint32)imm8 + (uint32)cf > 0xFF) regs.eflags |= 0x01; // CF
			bytes_consumed = prefix_offset + 2;
			return B_OK;
		}
		...
		case 0x1C:
		{
			...
			if (result == 0) regs.eflags |= 0x40; // ZF
			if ((int8)result < 0) regs.eflags |= 0x80; // SF
			if ((uint32)al < (uint32)imm8 + (uint32)cf) regs.eflags |= 0x01; // CF
			bytes_consumed = prefix_offset + 2;
			return B_OK;
		}
		...
		case 0x04:
		{
			...
			if (result == 0) regs.eflags |= 0x40; // ZF
			if ((int8)result < 0) regs.eflags |= 0x80; // SF
			if ((uint32)al + (uint32)imm8 > 0xFF) regs.eflags |= 0x01; // CF
			bytes_consumed = prefix_offset + 2;
			return B_OK;
		}
		...
		case 0x3A:
		{
			...
			if (mod == 3) {
				...
				if (result == 0) regs.eflags |= 0x40;  // ZF
				if ((int8)result < 0) regs.eflags |= 0x80;  // SF
				if ((uint8)dst < (uint8)src) regs.eflags |= 0x01;  // CF
		...
		case 0x0F:
		{
			...
			printf("[INTERPRETER] TWO-BYTE OPCODE: 0x0F 0x%02x - not fully implemented\n", second_opcode);
			...
			if (mod == 1) skip_len = 4;  // + disp8
			else if (mod == 2) skip_len = 7;  // + disp32
			else if (mod == 0 && rm == 4) skip_len++;  // + SIB
		...
		case 0x63:
		{
			...
			uint32 instr_len = 2;
			if (mod == 1) instr_len = 3;
			else if (mod == 2) instr_len = 6;
			else if (mod == 0 && (modrm & 7) == 4) instr_len = 3;  // SIB
			bytes_consumed = prefix_offset + instr_len;
			return B_OK;
		}
		...
		case 0x8A:
		{
			...
			if (mod == 3) {
				...
				*dst_reg[reg_field] = (*dst_reg[reg_field] & 0xFFFFFF00) | src_val;
				...
				else if (rm_field == 4) instr_len = 3;  // SIB
				bytes_consumed = prefix_offset + instr_len;
				return B_OK;
			}
		}
		...
		case 0x0C:
		{
			...
			if (al == 0) regs.eflags |= 0x40;  // ZF
			if ((int8)al < 0) regs.eflags |= 0x80;  // SF
		...
		case 0xDC: case 0xDD: case 0xDE: case 0xDF:
		{
			...
			status_t fpu_status = handler.Execute(fpu_opcode, modrm, x86_context, fAddressSpace);
		...
		case 0x11:
		{
			...
			if (mod == 3) {
				...
				*reg_ptrs[rm_field] = (uint32)result;
				...
				regs.eflags = 0;
				if ((uint32)result == 0) regs.eflags |= 0x40; // ZF
				if ((int32)result < 0) regs.eflags |= 0x80; // SF
				if (result > 0xFFFFFFFFULL) regs.eflags |= 0x01; // CF
				bytes_consumed = prefix_offset + 2;
				return B_OK;
			}
			// Memory mode not implemented
		...
		default:
		{
			...
			if (opcode < 0x40) {
				if (opcode & 1) {  // Odd opcodes usually have ModRM
					...
					if (mod == 1) skip_len = 3;  // + disp8
					if (mod == 2) skip_len = 6;  // + disp32
					if (rm == 4 && mod != 3) skip_len++;  // + SIB
				}
			}
			
			printf("[INTERPRETER] UNKNOWN OPCODE: 0x%02x at EIP=0x%08x (guessing %d bytes)\n", 
...
status_t InterpreterX86_32::Execute_GROUP_81(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	DebugPrintf("       GROUP1 0x81 Debug: ModR/M=0x%02x, mod=%u, reg_field=%u, rm_field=%u\n",
		modrm, mod, reg_field, rm_field);

	// Solo manejaremos el modo de registro a registro por ahora (Mod = 3)
	if (mod != 3) {
		DebugPrintf("       GROUP1 0x81: Memory addressing not supported yet (mod=%u)\n", mod);
		return B_BAD_DATA;
	}

	// El operando inmediato de 32 bits siempre sigue al ModR/M byte
	int32 imm32 = (int32)((instr[2] & 0xFF) |
	                       ((instr[3] & 0xFF) << 8) |
	                       ((instr[4] & 0xFF) << 16) |
	                       ((instr[5] & 0xFF) << 24));

	uint32* target_reg = reg_ptrs[rm_field]; // El operando destino es el r/m field en modo registro

	len = 6; // Opcode + ModR/M + Imm32

	DebugPrintf("       GROUP1 0x81: reg_field=%u (sub-opcode), rm_field=%u (target reg), imm32=%d\n",
...
status_t InterpreterX86_32::Execute_GROUP_83(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	DebugPrintf("       GROUP1 0x83 Debug: ModR/M=0x%02x, mod=%u, reg_field=%u, rm_field=%u\n",
		modrm, mod, reg_field, rm_field);

	// Solo manejaremos el modo de registro a registro por ahora (Mod = 3)
	if (mod != 3) {
		DebugPrintf("       GROUP1 0x83: Memory addressing not supported yet (mod=%u)\n", mod);
		return B_BAD_DATA;
	}
	...
	DebugPrintf("       GROUP1 0x83: reg_field=%u (sub-opcode), rm_field=%u (target reg), imm8=%d\n",
...
// /6 = (reserved)
// /7 = SAR r/m32, imm8
status_t InterpreterX86_32::Execute_GROUP_C1(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	printf("       GROUP_C1 Debug: ModR/M=0x%02x, mod=%u, reg_field=%u, rm_field=%u\n",
		modrm, mod, reg_field, rm_field);
	...
	if (is_memory) {
		...
		printf("       GROUP_C1: Memory addressing mode not fully supported (mod=%u, rm=%u)\n", mod, rm_field);
		
		// Estimate instruction length to skip
		uint32 skip_len = 2;  // opcode + modrm
		if (mod == 1) skip_len = 3;  // + disp8
		if (mod == 2) skip_len = 6;  // + disp32
		if (rm_field == 4) skip_len++;  // + SIB
	...
	printf("       GROUP_C1: reg_field=%u (sub-opcode), rm_field=%u (target reg), shift_count=%u\n",
	...
	switch (reg_field) {
		case 0: // ROL r/m32, imm8 (Rotate Left)
		{
			...
			if (shift_count > 0) {
				...
				uint32 rotated = (original_value << count) | (original_value >> (32 - count));
				
				if (target_ptr) *target_ptr = rotated;
		...
		case 4: // SHL/SAL r/m32, imm8 (Shift Left)
		{
			...
			if (target_ptr) *target_ptr = result;
			
			if (result == 0) regs.eflags |= FLAG_ZF; else regs.eflags &= ~FLAG_ZF;
			if (result & 0x80000000) regs.eflags |= FLAG_SF; else regs.eflags &= ~FLAG_SF;
		}
		break;
...
status_t InterpreterX86_32::Execute_MOV(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
		...
		if (mod == 3) {  // Register mode
			if (src_reg < 8 && dst_reg < 8) {
				*reg_ptrs[dst_reg] = *reg_ptrs[src_reg];
				len = 2;
				DebugPrintf("       %s <- %s (0x%08x)\n", reg_names[dst_reg], reg_names[src_reg], *reg_ptrs[dst_reg]);
				return B_OK;
			}
		}
...
status_t InterpreterX86_32::Execute_INT(GuestContext& context, const uint8* instr, uint32& len)
{
	...
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	printf("[INTERPRETER] EAX(syscall)=%u, EBX(arg1)=%u, ECX(arg2)=%u, EDX(arg3)=%u\n", 
		regs.eax, regs.ebx, regs.ecx, regs.edx);

	if (int_num == 0x80 || int_num == 0x25) {
		// Syscall interrupt
		// INT 0x80: Linux syscall convention
		// INT 0x25: Haiku syscall convention
...
status_t InterpreterX86_32::Execute_PUSH(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	if (opcode >= 0x50 && opcode <= 0x57) {
		...
		switch (reg_idx) {
			...
			case 1: value = regs.ecx; break;
			case 2: value = regs.edx; break;
			case 3: value = regs.ebx; break;
			case 4: value = regs.esp; break;
			case 5: value = regs.ebp; break;
			case 6: value = regs.esi; break;
			case 7: value = regs.edi; break;
		}
		len = 1;
	...
	if (status != B_OK) {
		...
		printf("       [REGISTER DUMP] EAX=0x%08x EBX=0x%08x ECX=0x%08x EDX=0x%08x\n", 
			regs.eax, regs.ebx, regs.ecx, regs.edx);
...
status_t InterpreterX86_32::Execute_PUSH_Imm(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_POP(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_ADD(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_SUB(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_CMP(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	if (opcode == 0x3D) {
		...
		if (reg < 8 && rm < 8) {
			...
			regs.eflags = 0;
			if (result == 0) regs.eflags |= 0x40; // ZF (bit 6)
			if ((int32)result < 0) regs.eflags |= 0x80; // SF (bit 7)
			if (result > 0x7FFFFFFF) regs.eflags |= 0x800; // OF (bit 11)
			
			DebugPrintf("       CMP R%d(0x%08x) vs R%d(0x%08x), FLAGS=0x%08x\n", 
				rm, *reg_ptrs[rm], reg, *reg_ptrs[reg], regs.eflags);
			return B_OK;
		}
	}
...
status_t InterpreterX86_32::Execute_XOR(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_JMP(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	if (opcode == 0xE9) {
		...
		if (modrm.mod == 3) {
			...
			if (status != B_OK) {
				DebugPrintf("       JMP [mem]: Failed to read target address from 0x%08x\n", mem_addr);
				return status;
			}
			...
			if (target_eip < 0x1000000) {  // Arbitrary threshold for "relative" addresses
				X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
				uint32_t image_base = x86_context.GetImageBase();
				target_eip += image_base;
				printf("       [FF JMP DEBUG] Adjusted relative offset: 0x%08x + 0x%08x = 0x%08x\n", 
					target_eip - image_base, image_base, target_eip);
			}
			
			printf("       [FF JMP DEBUG] Memory addressing: mem_addr=0x%08x, target_eip=0x%08x\n", 
				mem_addr, target_eip);
...
status_t InterpreterX86_32::Execute_RET(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	if (status != B_OK) {
		DebugPrintf("       RET: Failed to read return address from stack at 0x%08x\n", regs.esp);
		return status;
	}
	regs.esp += 4;
	len = 1;
...
status_t InterpreterX86_32::Execute_JZ(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_JNZ(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_JL(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_JLE(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_JG(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_JGE(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
...
status_t InterpreterX86_32::Execute_CALL(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	if (opcode == 0xE8) {
		...
		printf("       [FF CALL DEBUG] modrm.mod=%d, modrm.reg_op=%d, modrm.rm=%d\n", 
		...
		if (modrm.mod == 3) {
			...
			status_t status = fAddressSpace.Read(mem_addr, &target_eip, 4);
			if (status != B_OK) {
				DebugPrintf("       CALL [mem]: Failed to read target address from 0x%08x\n", mem_addr);
				return status;
			}
...
status_t InterpreterX86_32::DecodeModRM(const uint8* instr, ModRM& result)
{
	...
	printf("       [ModRM] mod=%d, reg_op=%d, rm=%d", result.mod, result.reg_op, result.rm);
	...
	if (result.rm == 4 && result.mod != 3) {
		...
		DebugPrintf(" [SIB: scale=%d, index=%s, base=%s]", 
			scale, 
...
uint32 InterpreterX86_32::GetEffectiveAddress(X86_32Registers& regs, const ModRM& modrm)
{
	uint32* reg_ptrs[] = {
		(uint32*)&regs.eax, (uint32*)&regs.ecx, (uint32*)&regs.edx, (uint32*)&regs.ebx,
		(uint32*)&regs.esp, (uint32*)&regs.ebp, (uint32*)&regs.esi, (uint32*)&regs.edi
	};
	...
	printf("       [Effective Addr] base=0x%08x (reg %d) + disp=0x%08x = 0x%08x\n",
...
status_t InterpreterX86_32::Execute_MOV_Load(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	if (modrm.mod == 3) {
		printf("       MOV Load: ERROR - mod=3 (register mode) should be handled elsewhere\n");
		return B_BAD_DATA;
	}
	...
	if (status != B_OK) {
		printf("       MOV Load: WARNING - Failed to read from address 0x%08x, using 0, status=%d\n", src_addr, status);
	...
	*reg_ptrs[dest_reg] = value;
	...
	DebugPrintf("       MOV %s, [0x%08x] (value=0x%08x)\n", 
		reg_names[dest_reg], src_addr, value);
...
status_t InterpreterX86_32::Execute_MOV_Store(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	DebugPrintf("       MOV [0x%08x], %s (value=0x%08x)\n",
	        dst_addr, reg_names[src_reg], src_value);
	...
	status_t InterpreterX86_32::Execute_MOV_Load_FS(GuestContext& context, const uint8* instr, uint32& len)
	{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	...
	// Check for special case: 0xA1 = MOV moffs32, eax (immediate offset, always EAX)
	uint8 opcode = instr[0];
	if (opcode == 0xA1) {
		...
		status_t status = fAddressSpace.ReadMemory(tls_address, &regs.eax, sizeof(uint32));
		if (status != B_OK) {
			printf("       ERROR: Failed to read TLS at offset 0x%08x (addr 0x%08x)\n", 
				offset, tls_address);
			return status;
		}
	...
	// NOTE: When FS override is present, the address calculation uses the normal addressing mode
	// but the access happens through the FS segment. In Haiku, FS base = TLS base (0xbffff000)
	// So the final address is: FS_BASE + (calculated effective address treated as offset)
	...
	if (modrm.mod == 3) {
	printf("       MOV Load FS: ERROR - mod=3 (register mode) shouldn't use FS override\n");
	return B_BAD_DATA;
	}
	...
	uint32* reg_ptrs_base[] = {
		(uint32*)&regs.eax, (uint32*)&regs.ecx, (uint32*)&regs.edx, (uint32*)&regs.ebx,
		(uint32*)&regs.esp, (uint32*)&regs.ebp, (uint32*)&regs.esi, (uint32*)&regs.edi
	};
	
	// For FS-relative access: typically just a displacement (mod=0, disp) or small offset
	// Example: MOV %fs:0x4, %eax  OR  MOV -24(%fs), %eax
	uint32 fs_offset = 0;
	if (modrm.mod == 0 && modrm.rm == 5) {
		...
		// Normal addressing with register base - for FS, treat register value as part of TLS address
		uint32 base = *reg_ptrs_base[modrm.rm];
		fs_offset = base + modrm.displacement;
		printf("       MOV Load FS: [reg+disp] reg=%d(0x%08x) + disp=0x%08x = offset=0x%08x\n",
	...
	printf("       MOV Load FS: fs_offset=0x%08x -> TLS_offset=0x%08x, address=0x%08x\n",
		fs_offset, tls_actual_offset, tls_address);
	
	uint32_t value;
	status = fAddressSpace.ReadMemory(tls_address, &value, sizeof(uint32));
	if (status != B_OK) {
	printf("       MOV Load FS: WARNING - Failed to read TLS at offset 0x%08x (addr 0x%08x), using 0\n",
	...
	*reg_ptrs[dest_reg] = value;
	...
	tls_actual_offset, reg_names[dest_reg], value);
...
template<typename T>
void InterpreterX86_32::SetFlags_ADD(X86_32Registers& regs, T result, T op1, T op2, bool is_32bit)
{
	// Clear relevant flags first
	regs.eflags &= ~(0x40 | 0x80 | 0x1 | 0x800 | 0x4 | 0x10); // Clear ZF, SF, CF, OF, PF, AF
	...
	// For addition: CF is set if there is a carry out of the most significant bit.
	if (is_32bit) {
	...
	// For addition: OF is set if signs of operands are same but sign of result is different.
...
status_t InterpreterX86_32::Execute_JA(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JAE(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JB(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JBE(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JP(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JNP(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JS(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JNS(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JO(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::Execute_JNO(GuestContext& context, const uint8* instr, uint32& len)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	X86_32Registers& regs = x86_context.Registers();
	int8 offset = (int8)instr[1];
...
status_t InterpreterX86_32::ExecuteStubFunction(GuestContext& context, uint32_t stub_address)
{
	X86_32GuestContext& x86_context = static_cast<X86_32GuestContext&>(context);
	...
	switch (stub_index) {
		// GNU coreutils and error handling
		case 0: status = StubFunctions::quote_quoting_options(x86_context, fAddressSpace); break;
		case 1: status = StubFunctions::close_stdout(x86_context, fAddressSpace); break;
		case 2: status = StubFunctions::version_etc_copyright(x86_context, fAddressSpace); break;
		case 3: status = StubFunctions::error_message_count(x86_context, fAddressSpace); break;
		case 4: status = StubFunctions::error_print_progname(x86_context, fAddressSpace); break;
		case 5: status = StubFunctions::program_name(x86_context, fAddressSpace); break;
		case 6: status = StubFunctions::exit_failure(x86_context, fAddressSpace); break;
		case 7: status = StubFunctions::thrd_exit(x86_context, fAddressSpace); break;
		case 8: status = StubFunctions::Version(x86_context, fAddressSpace); break;
		case 9: status = StubFunctions::error_one_per_line(x86_context, fAddressSpace); break;
		
		// GNU libc memory allocation wrappers
		case 10: status = StubFunctions::xmalloc(x86_context, fAddressSpace); break;
		case 11: status = StubFunctions::xcalloc(x86_context, fAddressSpace); break;
		case 12: status = StubFunctions::xrealloc(x86_context, fAddressSpace); break;
		case 13: status = StubFunctions::xcharalloc(x86_context, fAddressSpace); break;
		case 14: status = StubFunctions::xmemdup(x86_context, fAddressSpace); break;
		case 15: status = StubFunctions::x2nrealloc(x86_context, fAddressSpace); break;
		case 16: status = StubFunctions::xireallocarray(x86_context, fAddressSpace); break;
		case 17: status = StubFunctions::xreallocarray(x86_context, fAddressSpace); break;
		case 18: status = StubFunctions::ximalloc(x86_context, fAddressSpace); break;
		case 19: status = StubFunctions::xicalloc(x86_context, fAddressSpace); break;
		
		// GNU libc error functions
		case 20: status = StubFunctions::error(x86_context, fAddressSpace); break;
		case 21: status = StubFunctions::xalloc_die(x86_context, fAddressSpace); break;
		
		// GNU quoting functions (quotearg)
		case 22: status = StubFunctions::quotearg_alloc_mem(x86_context, fAddressSpace); break;
		case 23: status = StubFunctions::quotearg_n_custom_mem(x86_context, fAddressSpace); break;
		case 24: status = StubFunctions::quotearg_n_custom(x86_context, fAddressSpace); break;
		case 25: status = StubFunctions::quotearg_n_mem(x86_context, fAddressSpace); break;
		case 26: status = StubFunctions::quotearg_n(x86_context, fAddressSpace); break;
		case 27: status = StubFunctions::quotearg_char_mem(x86_context, fAddressSpace); break;
		case 28: status = StubFunctions::quotearg_char(x86_context, fAddressSpace); break;
		case 29: status = StubFunctions::quotearg_colon(x86_context, fAddressSpace); break;
		case 30: status = StubFunctions::quotearg_n_style(x86_context, fAddressSpace); break;
		case 31: status = StubFunctions::quotearg_n_style_mem(x86_context, fAddressSpace); break;
		case 32: status = StubFunctions::quote_n(x86_context, fAddressSpace); break;
		case 33: status = StubFunctions::quote_n_mem(x86_context, fAddressSpace); break;
		
		// GNU libc version/program functions
		case 34: status = StubFunctions::set_program_name(x86_context, fAddressSpace); break;
		case 35: status = StubFunctions::getprogname(x86_context, fAddressSpace); break;
		case 36: status = StubFunctions::version_etc(x86_context, fAddressSpace); break;
		case 37: status = StubFunctions::version_etc_arn(x86_context, fAddressSpace); break;
		case 38: status = StubFunctions::version_etc_va(x86_context, fAddressSpace); break;
		case 39: status = StubFunctions::usage(x86_context, fAddressSpace); break;
		
		// GNU libc locale/encoding functions
		case 40: status = StubFunctions::locale_charset(x86_context, fAddressSpace); break;
		case 41: status = StubFunctions::hard_locale(x86_context, fAddressSpace); break;
		case 42: status = StubFunctions::setlocale_null_r(x86_context, fAddressSpace); break;
		case 43: status = StubFunctions::rpl_nl_langinfo(x86_context, fAddressSpace); break;
		
		// rpl_* replacement functions
		case 44: status = StubFunctions::rpl_malloc(x86_context, fAddressSpace); break;
		case 45: status = StubFunctions::rpl_calloc(x86_context, fAddressSpace); break;
		case 46: status = StubFunctions::rpl_realloc(x86_context, fAddressSpace); break;
		case 47: status = StubFunctions::rpl_free(x86_context, fAddressSpace); break;
		case 48: status = StubFunctions::rpl_mbrtowc(x86_context, fAddressSpace); break;
		case 49: status = StubFunctions::rpl_fclose(x86_context, fAddressSpace); break;
		case 50: status = StubFunctions::rpl_fflush(x86_context, fAddressSpace); break;
		case 51: status = StubFunctions::rpl_fseeko(x86_context, fAddressSpace); break;
		case 52: status = StubFunctions::rpl_vfprintf(x86_context, fAddressSpace); break;
		
		// GNU quoting option functions
		case 53: status = StubFunctions::set_char_quoting(x86_context, fAddressSpace); break;
		case 54: status = StubFunctions::set_custom_quoting(x86_context, fAddressSpace); break;
		
		// GNU printf functions
		case 55: status = StubFunctions::printf_parse(x86_context, fAddressSpace); break;
		case 56: status = StubFunctions::printf_fetchargs(x86_context, fAddressSpace); break;
		case 57: status = StubFunctions::vasnprintf(x86_context, fAddressSpace); break;
		
		// GNU stream functions
		case 58: status = StubFunctions::fseterr(x86_context, fAddressSpace); break;
		case 59: status = StubFunctions::close_stream(x86_context, fAddressSpace); break;
		
		// GNU filesystem functions
		case 60: status = StubFunctions::globfree(x86_context, fAddressSpace); break;
		
		// Less common GNU functions from libc
		case 61: status = StubFunctions::gl_get_setlocale_null_lock(x86_context, fAddressSpace); break;
